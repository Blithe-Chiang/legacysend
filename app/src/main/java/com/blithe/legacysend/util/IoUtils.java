...
public final class IoUtils {
    ...
    public static long copy(InputStream input, OutputStream output, long expected,
        ...
        while (expected < 0 || copied < expected) {
            ...
            if (expected >= 0) wanted = (int) Math.min(wanted, expected - copied);
            if (wanted == 0) break;
            int read = input.read(buffer, 0, wanted);
            if (read < 0) break;
            output.write(buffer, 0, read);
            copied += read;
            if (listener != null) listener.onBytes(copied);
        }
        output.flush();
        if (expected >= 0 && copied != expected) {
            throw new IOException("连接提前中断：预期 " + expected + " 字节，实际 " + copied + " 字节");
        }
        return copied;
    }
